...
class MMRAG:
    def __init__(
        ...
        # load search_engine
        if (
            embed_model_name_vl is not None
            and embed_model_name_text is not None
        ):
            ...
            self.search_engine = SearchEngine(
                self.dataset, embed_model_name=embed_model_name
            )
    ...
    def vidorag(self, sample):
        ...
        candidate_image = [
            os.path.join(
                ...
                os.path.basename(
                    node["node"]["metadata"]
                    .get("file_name", node["node"]["metadata"].get("filename"))
                    .replace(".txt", ".jpg")
                ),
            )
    ...
    def eval_overall(self):
        ...
        with open(
            self.output_file_path.replace(".jsonl", "_eval.json"), "w"
        ) as f:
...
def arg_parse():
    ...
    parser.add_argument(
        "--query_file",
        type=str,
        default="rag_dataset.json",
        help="The name of anno_file",
    )
    parser.add_argument(
        "--experiment_type",
        type=str,
        default="retrieval_infer",  # retrieval_infer, dynamic_hybird_retrieval_infer, vidorag
    ...
    parser.add_argument(
        "--workers_num", type=int, default=1, help="The number of workers"
    )
    parser.add_argument(
        "--topk", type=int, default=10, help="The number of topk"
    )
    parser.add_argument(
        "--embed_model_name_vl",
        type=str,
        default=None,
        help="The name of embedding model for vl",
    )
    parser.add_argument(
        "--embed_model_name_text",
        type=str,
        default=None,
        help="The name of embedding model for text",
    )
    parser.add_argument(
        "--generate_vlm",
        type=str,
        default="qwen-vl-max",
        help="The name of VLM model",
    )
...
if __name__ == "__main__":
    ...
    rich.print("[green] Starting eval dataset [/green]")
    mmrag.eval_dataset()
    rich.print("[green] Starting eval overall [/green]")
    mmrag.eval_overall()
    rich.print("[green] Starting eval overall_type_wise [/green]")
