...
def gmm(
    recall_result: list[NodeWithScore],
    input_length: int = 20,
    max_valid_length: int = 10,
    min_valid_length: int = 5,
) -> List[NodeWithScore]:
    ...
    recall_result = [
        np.array(recall_result[:input_length])[labels == label].tolist()
    ...
    if len(valid_recall_result) > max_valid_length:
        ...
        second_valid_recall_result_len = min_valid_length - len(
            valid_recall_result
        )
        valid_recall_result.extend(
            recall_result[second_max_index][:second_valid_recall_result_len]
        )
...
class SearchEngine:
    def __init__(
    ...
    ):  # nvidia/NV-Embed-v2 "vidore/colqwen2-v0.1"
        Settings.llm = None
        self.gmm = False
        ...
        self.rag_dataset_path = os.path.join(
            self.dataset_dir, "rag_dataset.json"
        )
        self.workers = 1
        ...
        if "vidore" in embed_model_name or "openbmb" in embed_model_name:
            if self.vl_ret:
                self.vector_embed_model = VL_Embedding(
                    model=embed_model_name, mode="image"
                )
            else:
                self.vector_embed_model = VL_Embedding(
                    model=embed_model_name, mode="text"
                )
        else:
    ...
    def online_search(self, query, node_list, topk=9):
        ...
        vector_index = VectorStoreIndex(
            nodes,
            embed_model=self.vector_embed_model,
            show_progress=True,
            use_async=False,
        ...
        query_engine = RetrieverQueryEngine(
            retriever=vector_retriever, node_postprocessors=node_postprocessors
        )
        ...
        if max_workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                results = list(
                    tqdm(
                        executor.map(
                            parse_file, files, [self.node_dir] * len(files)
                        ),
                        total=len(files),
                    )
                )
    ...
    def load_query_engine(self):
        ...
        if self.vl_ret and "vidore" in self.embed_model_name:
            ...
            self.embedding_img = [
                ...
                for tensor in self.embedding_img
            ]
        else:
    ...
    def load_retriever_embed(self, nodes):
        ...
        vector_retriever = vector_index.as_retriever(
            similarity_top_k=self.recall_num
        )
    ...
    def search(self, query):
        if self.vl_ret and "vidore" in self.embed_model_name:
            ...
            scores = self.vector_embed_model.processor.score(
                query_embedding, self.embedding_img
            )
        ...
        if self.gmm_candidate_length:
            ...
            target_length = min(
                [num for num in candidate_length if num > current_length]
            )
    ...
    def search_multi_session(self, output_file="search_result.json"):
        ...
        if self.workers == 1:
            ...
            with ThreadPoolExecutor(max_workers=self.workers) as executor:
                future_to_file = {
                    executor.submit(self.search_example, example): example
                    for example in data
                }
                for future in tqdm(
...
class HybridSearchEngine:
    def __init__(
        ...
        self.engine_vl = SearchEngine(
            dataset,
            node_dir_prefix=node_dir_prefix_vl,
            embed_model_name=embed_model_name_vl,
        )
        self.engine_text = SearchEngine(
            dataset,
            node_dir_prefix=node_dir_prefix_text,
            embed_model_name=embed_model_name_text,
        )
        self.topk = topk
        self.gmm = gmm
    ...
    def search(self, query):
        ...
        if union_result:
            ...
            result_text["source_nodes"] = result_text["source_nodes"][
                : self.topk
            ]

            result_docs = dict()
            for node in result_vl["source_nodes"]:
                file = os.path.basename(node["node"]["image_path"]).split(".")[
                    0
                ]
            ...
            for key, pages in result_docs.items():
                ...
                for page in pages:
                    with open(
                        os.path.join(self.ppocr_dir, f"{key}_{page}.node"), "r"
                    ) as f:
                        text = json.load(f)
                    ...
                    node = ImageNode(
                        ...
                        text=text,
                        metadata=dict(file_name=file_path),
                    )
            ...
            for node in result_vl_gmm["source_nodes"]:
                ...
                file = ".".join(
                    os.path.basename(node["node"]["image_path"]).split(".")[:-1]
                )
                doc = "_".join(file.split("_")[:-1])
                page = file.split("_")[-1]
                if doc not in result_docs:
            ...
            for node in result_text_gmm["source_nodes"]:
                # file = node['node']['metadata']['filename'].split('.')[0]
                file = ".".join(
                    node["node"]["metadata"]["filename"].split(".")[:-1]
                )
                doc = "_".join(file.split("_")[:-1])
                page = file.split("_")[-1]
                if doc not in result_docs:
            ...
            overleap = [
                doc
                for doc in result_docs_vl_list
                if doc in result_docs_text_list
            ]

            candidate_length = [1, 2, 4, 6, 9, 12, 16, 20]
            already_length = sum(
                [len(value) for _, value in result_docs.items()]
            )
            target_length = min(
                (x for x in candidate_length if x >= already_length)
            )
            candidate_overleap = [
                node for node in overleap if node not in result_docs_list
            ][: target_length - already_length]
            candidate_overleap = candidate_overleap + [
                node
                for node in result_docs_vl_list
                if (node not in candidate_overleap)
                and (node not in result_docs_list)
            ]
            candidate_overleap = candidate_overleap[
                : target_length - already_length
            ]
