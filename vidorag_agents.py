...
class Seeker:
    ...
    def run(self, query=None, images_path=None, feedback=None):
        if query is not None and images_path is not None:
            ...
            self.query = query
            prompt = seeker_prompt.replace("{question}", self.query).replace(
                "{page_map}", self.page_map[len(self.buffer_images)]
            )
            ...
            prompt = seeker_prompt.replace(
                "{question}", additional_information
            ).replace("{page_map}", self.page_map[len(self.buffer_images)])
        ...
        if self.seeker_multi_image:
            ...
            input_images = [
                concat_images_with_bbox(
                    self.buffer_images,
                    arrangement=arrangement_map_dict[len(self.buffer_images)],
                    scale=1,
                    line_width=40,
                )
            ]
        ...
        while True:
            ...
            try:
                ...
                if (
                    ...
                    or any(
                        [
                            page >= len(self.buffer_images)
                            for page in select_page_num
                        ]
                    )
                ):
                    raise Exception(
                        f"select json format error: length: {len(self.buffer_images)}"
                    )

                selected_images = [
                    self.buffer_images[page] for page in select_page_num
                ]
                self.buffer_images = [
                    image
                    for image in self.buffer_images
...
class Inspector:
    ...
    def run(self, query, images_path):
        ...
        prompt = inspector_prompt.replace("{question}", query).replace(
            "{page_map}", self.page_map[len(self.buffer_images)]
        )
        ...
        while True:
            ...
            try:
                ...
                if reason is None:
                    ...
                    if (
                        any([page >= len(self.buffer_images) for page in ref])
                        or len(ref) == 0
                    ):
                    ...
                    if any(
                        [page >= len(self.buffer_images) for page in choice]
                    ):
                        raise Exception("choice error")
                    self.buffer_images = [
                        self.buffer_images[page] for page in choice
                    ]
...
class Synthesizer:
    ...
    def run(self, query, candidate_answer, ref_images):
        if candidate_answer is not None:
            query = query + "\n\n## Related Information\n" + candidate_answer
        prompt = answer_prompt.replace("{question}", query).replace(
            "{page_map}", self.page_map[len(ref_images)]
        )
        ...
        if self.synthesizer_multi_image:
            ...
            input_images = [
                concat_images_with_bbox(
                    ref_images,
                    arrangement=arrangement_map_dict[len(ref_images)],
                    scale=1,
                    line_width=40,
                )
            ]
...
class ViDoRAG_Agents:
    ...
    def run_agent(self, query, images_path):
        ...
        selected_images, summary, reason = self.seeker.run(
            query=query, images_path=images_path
        )
        # iter
        while True:
            status, information, images = self.inspector.run(
                query, selected_images
            )
            if status == "answer":
                return information
            elif status == "synthesizer":
                reason, answer = self.synthesizer.run(
                    query, information, images
                )
                return answer
            elif status == "seeker":
                selected_images, summary, reason = self.seeker.run(
...
if __name__ == "__main__":
    ...
    re = agent.run_agent(
        ...
        images_path=[
            ...
            "./data/ExampleDataset/img/00a76e3a9a36255616e2dc14a6eb5dde598b321f_3.jpg",
            "./data/ExampleDataset/img/00a76e3a9a36255616e2dc14a6eb5dde598b321f_4.jpg",
        ],
    )
    print(re)
